...
module inference #(
    ...
    output wire                    complete,
    output wire signed [31:0]      neuron_out [0:9],
...
//		,.y(y)
    ...
    logic signed [63:0] neurons_raw [0:9];
    ...
    // Saturating cast: clamp each 64-bit accumulator into signed 32-bit range
    genvar gi;
    generate
        for (gi = 0; gi < 10; gi++) begin : sat
            assign neuron_out[gi] =
                (neurons_raw[gi] >  64'sh000000007FFFFFFF) ? 32'sh7FFFFFFF :
                (neurons_raw[gi] < -64'sh0000000080000000) ? 32'sh80000000 :
                neurons_raw[gi][31:0];
        end
    endgenerate
